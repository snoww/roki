        ...
        public class ShowdownCommands : RokiSubmodule<ShowdownService>
        {
            ...
            public async Task BetPokemonGame(int gen = 7)
            {
                ...
                try
                {
                    ...
                    {
                        await _currency
                            .ChangeAsync(key.Id, "BetShowdown Entry", -value.Amount * value.Multiple, ctx.User.Id, ctx.Client.CurrentUser.Id, ctx.Guild.Id, ctx.Channel.Id,
                                ctx.Message.Id)
                            .ConfigureAwait(false);
                    }
                    ...
                    foreach (var (key, value) in joinedReactions)
                    {
                        var before = _service.GetCurrency(ctx.User.Id) + value.Amount * value.Multiple;
                        if (result != value.Bet)
                        {
                            losers += $"{key.Username,-15} " +
                        ...
                        winners += $"{key.Username + "won `" + won.ToString("N0") + "` " + _roki.Properties.CurrencyIcon,-15}" +
                                   $"{"`" + before.ToString("N0") + "` ⇒ `" + _service.GetCurrency(ctx.User.Id).ToString("N0"), 20}`\n";
                    }
